...
#define LIST_HEAD_REQ(LIST, OBJ) \
    do {\
        if ((LIST)->head != NULL){\
            (OBJ)->reserved = ((robj*)((LIST)->head->value))->reserved - 1;\
        } else {\
            (OBJ)->reserved = UINT_MAX / 2;\
        }\
    } while(0)

#define LIST_TAIL_REQ(LIST, OBJ) \
    do {\
        if ((LIST)->tail != NULL){\
            (OBJ)->reserved = ((robj*)((LIST)->tail->value))->reserved + 1;\
        } else {\
            (OBJ)->reserved = UINT_MAX / 2;\
        }\
    } while(0)
...
void listTypePush(robj *subject, robj *value, int where)
{
    ...
    else if (subject->encoding == REDIS_ENCODING_LINKEDLIST)
    {
        if (where == REDIS_HEAD)
        {
            LIST_HEAD_REQ((list*)subject->ptr, value);
            listAddNodeHead(subject->ptr,value);
        }
        else
        {
            LIST_TAIL_REQ((list*)subject->ptr, value);
...
void pushGenericCommand(redisClient *c, int where)
{
    ...
    robj **argv = zcalloc(c->argc * sizeof(robj*));
    int argc = 0;
    ...
    if (argc > 0)
    {
        dbmng_save_op(c->tag, c->argv[0]->ptr, c->argv[1], argc, argv, c->ds_id, c->db->id);
    }
    //log_prompt("push: write binlog...argc=%d, key=%s, where=%d, cmd=%s"
    //        , c->argc, c->argv[1]->ptr, where, c->argv[0]->ptr);
    zfree(argv);
    argv = 0;
}
...
void pushxGenericCommand(redisClient *c, robj *refval, robj *val, int where)
{
    ...
    if (refval != NULL)
    {
        ...
        if (inserted)
        {
            ...
            dbmng_save_op(c->tag, OP_LINSERT, c->argv[1], c->argc - 2, &c->argv[2], c->ds_id, c->db->id);
        }
        else
        {
    ...
    else
    {
        ...
        const char *const op = (where == REDIS_TAIL) ? OP_LRPUSH : OP_LLPUSH;
        dbmng_save_op(c->tag, op, c->argv[1], c->argc - 2, &c->argv[2], c->ds_id, c->db->id);
    }
...
void lsetCommand(redisClient *c)
{
    ...
    if (o->encoding == REDIS_ENCODING_ZIPLIST)
    {
        ...
        else
        {
            ...
            dbmng_save_op(c->tag, OP_LSET, c->argv[1], c->argc - 2, &c->argv[2], c->ds_id, c->db->id);
        }
    }
    else if (o->encoding == REDIS_ENCODING_LINKEDLIST)
    {
...
void popGenericCommand(redisClient *c, int where)
{
    ...
    {
        char buf[32];
        sprintf(buf, "%u", value->reserved);
        robj *seq_obj = createStringObject(buf, strlen(buf));
        ...
        const char *const op = (where == REDIS_TAIL) ? OP_LRPOP : OP_LLPOP;
        dbmng_save_op(c->tag, op, c->argv[1], 1, &seq_obj, c->ds_id, c->db->id);
        decrRefCount(seq_obj);
    }
}

void lpopCommand(redisClient *c)
{
    //log_prompt("lpop...key=%s", c->argv[1]->ptr);
    popGenericCommand(c,REDIS_HEAD);
}

void rpopCommand(redisClient *c)
{
    //log_prompt("rpop...key=%s", c->argv[1]->ptr);
...
void ltrimCommand(redisClient *c)
{
    ...
    dbmng_save_op(c->tag, OP_LTRIM, c->argv[1], c->argc - 2, &c->argv[2], c->ds_id, c->db->id);
}
...
void lremCommand(redisClient *c)
{
    ...
    dbmng_save_op(c->tag, OP_LREM, c->argv[1], c->argc - 2, &c->argv[2], c->ds_id, c->db->id);
}
...
void rpoplpushHandlePush(redisClient *origclient, redisClient *c, robj *dstkey, robj *dstobj, robj *value)
{
    ...
    if (!handleClientsWaitingListPush(origclient,dstkey,value))
    {
        ...
        dbmng_save_op(origclient->tag, origclient->argv[0]->ptr, origclient->argv[1], 1, &origclient->argv[2], origclient->ds_id, origclient->db->id);
    }
