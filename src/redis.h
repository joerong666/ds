...
#define REDIS_DBE_GET_WAIT 512 /* This client is waiting for dbe get */ 
#define REDIS_WR_BL_WAIT 1024 /* This client is waiting for write binlog */ 
...
/* Log levels */
#define REDIS_DEBUG PF_LOG_DEBUG
#define REDIS_VERBOSE PF_LOG_INFO
#define REDIS_NOTICE PF_LOG_WARN
#define REDIS_WARNING PF_LOG_ERROR
#define REDIS_PROMPT PF_LOG_PROMPT
...
typedef struct redisObject {
    ...
    unsigned rsvd_bit:1;    // field reserved valid or not
    unsigned visited_bit:1;
    unsigned ts_bit:1;      // field timestamp valid or not
    unsigned no_used:29;    // no used any more
    uint32_t reserved;      // "flag" field for mc or sequence for list of redis
    uint64_t version;       // for cas of mc
    uint64_t timestamp;     // for sync on cache mode
...
typedef struct redisClient
{
    ...
    list *dbe_get_keys;     /* Keys this client is waiting for getting from dbe */
    ...
    int master; /* client connect to master port or slave port */
    int tid;
    int read_only;
    int stat; /* 0 - conn init, 1 - reuse & query null, 2 - reuse & queue avail */
    long long bl_tag;
    unsigned long long ds_id;

    long long block_start_time;
    long long start_time;
    long long recv_dur;
    long long call_dur;
    long long dbe_get_block_dur;
    long long wr_bl_block_dur;
    int64_t wr_bl_dur;
    int req_len;
    void *ctx;
    int enque_cnt;
...
struct sharedObjectsStruct {
    ...
    *colon, *nullbulk, *nullmultibulk, *queued, *nosupport,
...
#define DBE_VER_UNKNOWN                0
#define DBE_VER_HIDB                   1
#define DBE_VER_HIDB2                  2
...
struct redisServer
{
    ...
    int recv_fd; /* pipe[0] */
    int send_fd; /* pipe[1] */
    int hb_recv_fd; /* pipe[0] for hb */
    int hb_send_fd; /* pipe[1] for hb */
    redisDb *db;
    ...
    long long stat_cmd_total_dur;   /* total duration of processed commands */
    ...
    unsigned long long stat_get_cmd;
    unsigned long long stat_set_cmd;
    unsigned long long stat_touch_cmd;
    unsigned long long stat_incr_cmd;
    unsigned long long stat_decr_cmd;
    unsigned long long stat_del_cmd;
    unsigned long long stat_auth_cmd;

    unsigned long long stat_get_hits;
    unsigned long long stat_get_misses;
    unsigned long long stat_touch_hits;
    unsigned long long stat_touch_misses;
    unsigned long long stat_incr_hits;
    unsigned long long stat_incr_misses;
    unsigned long long stat_decr_hits;
    unsigned long long stat_decr_misses;
    unsigned long long stat_cas_hits;
    unsigned long long stat_cas_misses;
    unsigned long long stat_cas_badval;
    unsigned long long stat_del_hits;
    unsigned long long stat_del_misses;
    unsigned long long stat_auth_errors;

    unsigned long long stat_expired_unfetched;
    unsigned long long stat_evicted_unfetched;

    unsigned long long stat_dbeio_get;
    unsigned long long stat_dbeio_put;
    unsigned long long stat_dbeio_del;
    unsigned long long stat_dbeio_it;

    unsigned long long bytes_read;
    unsigned long long bytes_written;
    unsigned long long rejected_conns;
    ...
    char starttime_str[24];
    char app_id[16];
    unsigned int has_cache:1; /* 0: no cache; 1: yes */
    unsigned int has_dbe:1; /* 0: no dbe; 1: yes */
    unsigned int read_only:1; /* 0: read & write; 1: readonly */
    unsigned int is_slave:1; /* 0: master; 1: slave */
    unsigned int wr_bl:1; /* 0: don't write binlog; 1: write binlog */
    unsigned int slow_log:1; /* 0: not , 1: yes */
    unsigned int log_get_miss:1; /* 0: not , 1: yes */
    unsigned int read_dbe:1; /* 0: don't try dbe when un-match cache, 1: try dbe */
    unsigned int init_ready:1; /* 0: initing, 1: is ready */
    unsigned int load_bl:1; /* 0: don't load bl, 1: load bl; for cache */
    unsigned int auto_purge:1; /* 0: disable auto_purge; 1: enable */
    unsigned int ip_list_changed:1;
    unsigned int dbe_hot_level:4;
    char *ds_key; /* use for hash */
    int64_t db_max_size;
    int64_t db_min_size;
    long long load_hot_key_max_num; /* -1: unlimit, >0: max mumber */
    int load_bl_cnt; /* for cache */
    int dbe_fsize;

    /* static */
    unsigned long long stat_lru_del_keys;     /* number of keys deleted by lru */
    unsigned long long stat_misses_in_cache;  /* for persistence */
    unsigned long long stat_notify_dbe_get_fail;   /* notify dbe get fail */

    unsigned long long ds_key_num;

    int bl_sync_delay_max;
    long long bl_sync_delay_total;
    long long bl_sync_cnt;

    unsigned char accepting_conns;
    int querybuf_reuse;
    char *mngr_list;
    char *host; /* ip or domain name of this ds */

    list *repl_slaves;
    list *cleans;
    list *dbe_get_clients;
    list *write_bl_clients;
    list *bl_writing;
    list *wr_bl_list;
    list *wr_dbe_list;
    int dbe_get_clients_cnt;
...
struct redisCommand {
    ...
    int vm_firstkey; /* The first argument that's a key (0 = no keys) for vm */
    int vm_lastkey;  /* THe last argument that's a key for vm */
    int vm_keystep;  /* The step between first and last key for vm */

    int io_firstkey; /* The first argument that's a key (0 = no keys) for dbe_get */
    int io_lastkey;  /* THe last argument that's a key for dbe_get */
    int io_keystep;  /* The step between first and last key for dbe_get */

    int firstkey; /* The first argument that's a key (0 = no keys) */
    int lastkey;  /* THe last argument that's a key */
    int keystep;  /* The step between first and last key */

    int wrbl_flag; /* the flag whether need to write binlog */
    size_t visit_cnt;
    char type;    /* for fooyun: cmd type */
};
...
#if 1 /* Added by zzn: not use shared integers */
...
#define redisLog(level, fmt, ...) redisLog_(__FILE__, __LINE__, level, fmt, ##__VA_ARGS__);
void usage();
...
typedef struct sync_addr_t
{
    char ip[64];
    int port;
} sync_addr;
extern int start_syncs(sync_addr *masters, int cnt);
extern int start_sync(const char *master_ip, int master_port, long long bl_tag, int conf, int ignore, int mode);
extern void push_back_to_clean_list(redisClient *c);

extern size_t g_tid;  /* connection id */
extern int g_tag;
extern int g_max_fd;
